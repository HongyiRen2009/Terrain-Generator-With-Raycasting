...
/***/ "./src/modelLoader/ThreeMFWorker.ts"
/*!******************************************!*\
  !*** ./src/modelLoader/ThreeMFWorker.ts ***!
  \******************************************/
...
function parseTransform(transformString) {
    const transform = gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create();
    if (!transformString)
        return transform;
    try {
        ...
        if (parts.length < 12)
            return transform;
        gl_matrix__WEBPACK_IMPORTED_MODULE_1__.set(transform, parts[0], parts[1], parts[2], 0, parts[3], parts[4], parts[5], 0, parts[6], parts[7], parts[8], 0, parts[9], parts[10], parts[11], 1);
    }
    catch {
        return transform;
    }
...
function parse3MFModelWorker(xmlString) {
    ...
    if (unitLower === "meter")
        scale = 1000.0;
    else if (unitLower === "inch")
        scale = 25.4;
    else if (unitLower === "centimeter")
        scale = 10.0;
    else if (unitLower === "micrometer")
        scale = 0.001;
    else if (unitLower === "foot")
        scale = 304.8;
    ...
    let combinedTransform = gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create();
        ...
        if (itemValue) {
            ...
            items.forEach((item, idx) => {
                ...
                if (idx === 0) {
                    ...
                    hasTransform = !gl_matrix__WEBPACK_IMPORTED_MODULE_1__.equals(transform, gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create());
                }
                else {
                    gl_matrix__WEBPACK_IMPORTED_MODULE_1__.multiply(combinedTransform, combinedTransform, transform);
                }
                stack.push({ id: objectId, transform: gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create() });
            });
            while (stack.length > 0) {
                ...
                if (obj.mesh) {
                    ...
                    for (const v of mesh.vertices) {
                        const transformed = gl_matrix__WEBPACK_IMPORTED_MODULE_3__.create();
                        gl_matrix__WEBPACK_IMPORTED_MODULE_3__.transformMat4(transformed, v, transform);
                    ...
                    for (const tri of mesh.triangles) {
                        finalTriangles.push(tri.indices[0] + vertexOffset, tri.indices[1] + vertexOffset, tri.indices[2] + vertexOffset);
                    }
                }
                else if (obj.components) {
                    for (let i = obj.components.length - 1; i >= 0; i--) {
                        const c = obj.components[i];
                        const childTransform = gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create();
                        gl_matrix__WEBPACK_IMPORTED_MODULE_1__.multiply(childTransform, transform, c.transform);
    ...
    return {
        ...
        transform: hasTransform ? combinedTransform : gl_matrix__WEBPACK_IMPORTED_MODULE_1__.create()
    };
}
function parseMaterialResources(resources, library) {
    ...
    for (const group of groups) {
        ...
        if (!id)
            continue;
        const material = {
            metallic: parseFloat(getValue(group, "@_metallic", "@_metalness", "@_metal") || "0"),
            roughness: parseFloat(getValue(group, "@_roughness", "@_rough") || "0.5"),
            specular: parseFloat(getValue(group, "@_specular", "@_spec") || "0.5")
        };
        library.materials.set(id, material);
    }
}
function parseColorResources(resources, library, defaultColor) {
    ...
    for (const group of groups) {
        ...
        for (const c of colorArray) {
            ...
            parsedColors.push(gl_matrix__WEBPACK_IMPORTED_MODULE_3__.clone(defaultColor));
        }
...
function parseObjects(resources, library, scale, defaultColor) {
...
function parseMesh(objectNode, library, scale, defaultColor) {
    ...
    if (verticesNode) {
        ...
        for (const v of vertexArray) {
            ...
            vertices.push(gl_matrix__WEBPACK_IMPORTED_MODULE_3__.fromValues(x * scale, y * scale, z * scale));
        }
    }
    const objectColors = new Array(vertices.length);
    const objectMaterials = new Array(vertices.length);
    for (let i = 0; i < vertices.length; i++) {
        objectColors[i] = gl_matrix__WEBPACK_IMPORTED_MODULE_3__.clone(defaultColor);
        objectMaterials[i] = {
            metallic: 0,
            roughness: 0.5,
            specular: 0.5
    ...
    if (trianglesNode) {
        ...
        for (const t of triangleArray) {
            ...
            if (trianglePid && library.colors.has(trianglePid)) {
                ...
                if (p1 < colors.length)
                    objectColors[v1] = colors[p1];
                if (p2 < colors.length)
                    objectColors[v2] = colors[p2];
                if (p3 < colors.length)
                    objectColors[v3] = colors[p3];
            }
    ...
    return {
        ...
        triangles,
        colors: objectColors,
        uvs: new Array(vertices.length).fill(gl_matrix__WEBPACK_IMPORTED_MODULE_2__.create()),
        materials: objectMaterials
    };
}
function parseComponents(componentsNode) {
    let componentArray;
...
async function load3MFWorker(url) {
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error(`Failed to fetch: ${response.statusText}`);
    }
    const fileData = await response.arrayBuffer();
    const zip = await jszip__WEBPACK_IMPORTED_MODULE_0___default().loadAsync(fileData);
...
self.onmessage = async (event) => {
    ...
    try {
        ...
        for (let i = 0; i < vertexCount; i++) {
            ...
            const c = model.colors[i] ?? gl_matrix__WEBPACK_IMPORTED_MODULE_3__.fromValues(0.8, 0.8, 0.8);
...
/******/ 		var __webpack_exports__ = __webpack_require__.O(undefined, ["vendors-node_modules_gl-matrix_esm_mat4_js-node_modules_gl-matrix_esm_vec2_js-node_modules_gl-2a3385"], () => (__webpack_require__("./src/modelLoader/ThreeMFWorker.ts")))
...
/******/ 	(() => {
/******/ 		__webpack_require__.hmrF = () => ("3e52e570dd1e1d1dc3f9." + __webpack_require__.h() + ".hot-update.json");
/******/ 	})();
/******/ 	
/******/ 	/* webpack/runtime/getFullHash */
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("c97009d0ba8519c4fad5")
/******/ 	})();
/******/ 	
...
/******/ 			"src_modelLoader_ThreeMFWorker_ts": 1
/******/ 		};
/******/ 		
...
/******/ 		__webpack_require__.x = () => {
/******/ 			return __webpack_require__.e("vendors-node_modules_gl-matrix_esm_mat4_js-node_modules_gl-matrix_esm_vec2_js-node_modules_gl-2a3385").then(next);
/******/ 		};
/******/ 	})();
/******/ 	
/************************************************************************/
...
/******/ })()
;
//# sourceMappingURL=src_modelLoader_ThreeMFWorker_ts.index.js.map