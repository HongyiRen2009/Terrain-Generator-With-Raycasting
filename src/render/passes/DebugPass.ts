...
export class DebugPass extends RenderPass {
  ...
  public VAOInputType: VAOInputType = VAOInputType.NONE;
  ...
  private static readonly NDC_CORNERS: vec4[] = (() => {
    ...
    for (let x = 0; x < 2; ++x) {
      for (let y = 0; y < 2; ++y) {
        for (let z = 0; z < 2; ++z) {
          corners.push(
            vec4.fromValues(2.0 * x - 1.0, 2.0 * y - 1.0, 2.0 * z - 1.0, 1.0)
          );
        }
      }
    }
  ...
  private static readonly LIGHT_FRUSTUM_NEAR_PLANE_COLOR: [
    number,
    number,
    number
  ] = [1.0, 0.2, 0.2];
  ...
  constructor(
    ...
    this.uniforms = getUniformLocations(gl, this.program!, ["viewProj"]);
  ...
  private initBuffers(): void {
    ...
    if (!this.positionBuffer || !this.colorBuffer || !this.vao) {
      throw new Error(
        "[DebugPass] Failed to allocate buffers for frustum debug rendering"
      );
    }
  ...
  public render(vaosToRender: VaoInfo[], pathtracerOn: boolean): void {
    const drawCascadeDebug = this.resourceCache.getData("drawCascadeDebug") as
      | boolean
      | undefined;
    const lightDebugCubes = this.resourceCache.getData("lightDebugCubes") as
      | LightDebugCube[]
      | undefined;
    const shouldRender =
      !!drawCascadeDebug || (lightDebugCubes && lightDebugCubes.length > 0);
    if (!shouldRender) {
      return;
    }
  ...
  private buildLineBuffers(
  ...
  ): { positions: number[]; colors: number[]; vertexCount: number } | null {
    ...
    if (includeCascadeDebug) {
      const cameraFrustum = this.resourceCache.getData(
      ...
      const cameraSubFrusta = this.resourceCache.getData("cameraSubFrusta") as
        | number[][][]
        | undefined;
      if (cameraSubFrusta && cameraSubFrusta.length > 0) {
        ...
        for (let i = 0; i < maxCascades; i++) {
          const color =
            DebugPass.CASCADE_COLORS[i % DebugPass.CASCADE_COLORS.length];
          pushFrustum(cameraSubFrusta[i], color);
        }
      }

      const lightSpaceMatrices = this.resourceCache.getData(
        "lightSpaceMatrices"
      ) as mat4[] | undefined;
      if (lightSpaceMatrices && lightSpaceMatrices.length > 0) {
        const cascadeIndex = Math.max(
          ...
          Math.min(
            ...
            (this.resourceCache.getData("shadowMapCascade") as
              | number
              | undefined) ?? 0
          )
        );
        const cascadeCorners = this.computeCascadeWorldCorners(
  ...
  private computeCascadeWorldCorners(
