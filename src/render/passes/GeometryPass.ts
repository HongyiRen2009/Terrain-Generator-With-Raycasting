...
export class GeometryPass extends RenderPass {
    ...
    constructor(gl: WebGL2RenderingContext, resourceCache: ResourceCache, canvas: HTMLCanvasElement, renderGraph?: RenderGraph) {
        ...
        this.canvas = canvas;
        this.program = RenderUtils.CreateProgram(gl, geometryVertexShaderSource, geometryFragmentShaderSource)!;
        this.renderTarget = this.initRenderTarget();
        this.uniforms = getUniformLocations(gl, this.program!, ["view", "proj", "model"]);
    }
    ...
    protected initRenderTarget(width?: number, height?: number): RenderTarget {
        ...
        const normalTexture = TextureUtils.createTexture(this.gl, w, h, this.gl.RGBA16F, this.gl.RGBA, this.gl.FLOAT);
        const albedoTexture = TextureUtils.createTexture(this.gl, w, h, this.gl.RGBA8, this.gl.RGBA, this.gl.UNSIGNED_BYTE);
        const depthTexture = TextureUtils.createTexture(this.gl, w, h, this.gl.DEPTH_COMPONENT32F, this.gl.DEPTH_COMPONENT, this.gl.FLOAT);
