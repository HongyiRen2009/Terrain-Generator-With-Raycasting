...
import { TerrainGeometryVertexShaderCode, TerrainGeometryFragmentShaderCode } from "../../glsl/geometry";
...
export class geometryPass {
    ...
    constructor(gl: WebGL2RenderingContext) {
        ...
        this.TerrainGeometryProgram = glUtils.CreateProgram(
            gl,
    ...
    DefferedRenderingGeometryPass(TransformationMatrix: mat4) {
        ...
        const indexBuffer = this.gl.getParameter(this.gl.ELEMENT_ARRAY_BUFFER_BINDING);
        if (indexBuffer) {
            const bufferSize = this.gl.getBufferParameter(this.gl.ELEMENT_ARRAY_BUFFER, this.gl.BUFFER_SIZE);
            const indexCount = bufferSize / 4; // 4 bytes per Uint32
            console.log(`Index buffer size: ${bufferSize} bytes, ${indexCount} indices`);
            console.log(`Requesting to draw ${this.TerrainMeshSize} elements`);
            
            if (this.TerrainMeshSize > indexCount) {
                console.error(`ERROR: Trying to draw ${this.TerrainMeshSize} elements but buffer only has ${indexCount} indices!`);
            }
        }
        ...
        console.log(`About to draw ${this.TerrainMeshSize} elements with UNSIGNED_INT type`);
