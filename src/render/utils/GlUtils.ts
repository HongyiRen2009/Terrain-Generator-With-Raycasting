...
export class glUtils {
  ...
  static CreateStaticBuffer(
    ...
    console.log(`Creating vertex buffer with ${CPUVertexBuffer.length} floats (${CPUVertexBuffer.length / 9} vertices)`);
  ...
  static createInterleavedVao(
    ...
    locations?: { [attribName: string]: number }
  ) {
    ...
    for (const [name, layoutInfo] of Object.entries(layout)) {
      ...
      let location = layoutInfo.location;
      if (location === undefined && locations) {
        location = locations[name];
      }
      if (location === undefined) {
        console.warn(`No location specified for attribute ${name}, skipping.`);
        continue;
      }
