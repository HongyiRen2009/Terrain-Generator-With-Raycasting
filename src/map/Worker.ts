...
function noiseFunction(c: vec3, simplex: NoiseFunction3D): number {
  ...
  const hillFreq = 0.015;
  const mountainFreq = 0.003;
  const caveFreq = 0.06;
  ...
  const hillNoise = fractalNoise(c, 5, hillFreq, 1);
  const hillHeight = hillNoise * (height * 0.6) + height * 0.2;

  const ridgeVal = 1 - Math.abs(fractalNoise(c, 4, mountainFreq, 1));
  let mountainHeight = Math.pow(ridgeVal, 2) * (height * 0.8);

  const spikeNoise = fractalNoise(c, 3, 0.01, 1);
  if (spikeNoise > 0.7) mountainHeight += (spikeNoise - 0.7) * height * 0.5;

  let terrainHeight = hillHeight * 0.4 + mountainHeight * 0.6;

  const riverNoise = fractalNoise(c, 3, 0.008, 1);
  if (riverNoise > 0.48 && riverNoise < 0.52) terrainHeight -= height * 0.1;

  const caveNoise = fractalNoise(c, 3, caveFreq, 1);
  let density = terrainHeight - c[1];
  density -= Math.max(
    0,
    (caveNoise - 0.5) * height * 0.8 * Math.max(0, 1 - c[1] / height)
  );

  if (c[1] < waterLevel) density = Math.min(density, waterLevel - c[1]);

  const normalized = (density + height) / (height * 2);
