...
class PathTracer{
    ...
    private compileShaders(){
        ...
        this.vertexShader = this.compileShaderType(this.context.VERTEX_SHADER,"");
        this.fragmentShader = this.compileShaderType(this.context.FRAGMENT_SHADER,"");
    }
    ...
    public constructor(canvas: HTMLCanvasElement, context: WebGL2RenderingContext){
        this.canvas = canvas;
    ...
    public render(time: number){
        ...
        if (true){ // TODO: Some future check to turn off pathtracing
            requestAnimationFrame(this.render);
        }
    }
}